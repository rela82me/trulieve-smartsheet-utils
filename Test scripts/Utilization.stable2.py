...
def fetch_smartsheet_data_for_analysis(context: ProcessingContext, sheet_id: int) -> Optional[Any]:
    ...
    try:
        ...
        logging.warning(
            f"API Error fetching sheet {sheet_id}: Code {e.error_code} - {e.message}")
...
def calculate_sheet_metrics(sheet_obj: Any, sheet_info: SheetInfo) -> SheetMetrics:
    sname = sheet_obj.name if hasattr(
        sheet_obj, 'name') else sheet_info.stub.name
    ...
    filled_cells = sum(
        1 for r in sheet_obj.rows for c in r.cells if c.value not in (None, "", []))
    ...
    ref_cnt = len(sheet_obj.cross_sheet_references) if hasattr(
        sheet_obj, 'cross_sheet_references') else 0
    ref_util = (
    ...
    return SheetMetrics(
        ...
        effective_attachments=getattr(sheet_obj.summary, 'effective_attachment_count', None) if hasattr(
            sheet_obj, 'summary') else None,
        ...
        resource_management_enabled=getattr(
            sheet_obj, 'resource_management_enabled', None),
        from_template=(getattr(sheet_obj.source, 'type', None) == 'template') if hasattr(
            sheet_obj, 'source') and sheet_obj.source else None
    )
...
def worker_analyze_sheet(context: ProcessingContext, sheet_info: SheetInfo) -> SheetMetrics:
    ...
    if sheet_obj:
        try:
            ...
            logging.error(
                f"Error calculating metrics for sheet {sheet_info.stub.name} ({sheet_info.stub.id}): {e}\n{traceback.format_exc()}")
                ...
                try:
                    ...
                    for sheet_stub in getattr(ws, "sheets", []):
                        ...
                        all_discovered_sheets.append(SheetInfo(
                            ...
                            access_level=str(
                                getattr(sheet_stub, 'access_level', None))
                        ))
                    if context.shutdown_event.is_set() or (discovery_limit is not None and len(all_discovered_sheets) >= discovery_limit):
                        break
...
def process_sheets_concurrently(context: ProcessingContext, sheets_to_analyze: List[SheetInfo], max_workers: int) -> List[SheetMetrics]:
    ...
    logging.info(
        f"Analyzing {len(sheets_to_analyze)} sheets using up to {max_workers} workers...")

    pbar_kwargs = {"disable": context.quiet_mode, "total": len(
        sheets_to_analyze), "desc": "Analyzing Sheets", "unit": "sheet"}
                ...
                for future in as_completed(future_to_sheet_info):
                    ...
                    try:
                        ...
                        if res.error and "cancelled by user" in res.error and not context.quiet_mode:
                            tqdm.write(
                                f"🚫 Sheet {res.sheet_name} analysis cancelled.")
                    except Exception as exc:
                        s_info_item = future_to_sheet_info[future]
                        logging.critical(
                            f"Critical error processing future for sheet {s_info_item.stub.name}: {exc}\n{traceback.format_exc()}")
                        error_metrics_payload = {
                            field: None for field in SheetMetrics._fields
                        }
                        error_metrics_payload.update({
                        ...
                        analyzed_results.append(
                            SheetMetrics(**error_metrics_payload))
...
def save_report_to_excel(context: ProcessingContext, output_filename: str, metrics_results: List[SheetMetrics]):
    ...
    try:
        with pd.ExcelWriter(final_outfile, engine='xlsxwriter') as writer:
            ...
            metric_column_order = [
                field_name for field_name in SheetMetrics._fields if field_name != 'error']
            if not df_metrics.empty:
                ...
                df_metrics.to_excel(
                    writer, sheet_name="Sheet Metrics", index=False)
            ...
            error_column_order = ["Sheet Name", "Sheet ID",
    ...
    if num_sheets_analyzed > 0:
        ...
        if 0 < len(high_row_util_sheets) <= 10:
            for _, row in high_row_util_sheets.iterrows():
                summary.append(
                    f"  - {row['sheet_name']} ({row['row_util_percent']:.2%})")
    summary.append("-----------------------------")
    logging.info("\n".join(summary))
...
def main():
    ...
    try:
        sheets_to_analyze = discover_sheets_in_workspaces(
        ...
        if processing_ctx.shutdown_event.is_set() and not sheets_to_analyze:
            ...
            logging.info(
                "No sheets found to analyze based on criteria. Exiting.")
            if processing_ctx.error_logs:
                save_report_to_excel(processing_ctx, cli_args.output, [])
            return
        ...
        analyzed_sheet_metrics = process_sheets_concurrently(
            processing_ctx, sheets_to_analyze, cli_args.max_workers)
        ...
        if analyzed_sheet_metrics or processing_ctx.error_logs:
            save_report_to_excel(
                processing_ctx, cli_args.output, analyzed_sheet_metrics)
        else:
