"""
PROFESSIONAL GRADE REFACTOR + .env Support + Fixes v5 (User Experience Focus)
Utilization.py
"""
...
from datetime import datetime as python_datetime, timezone, timedelta
import sys
...
class AssetMetrics(NamedTuple):
    ...
    created_at: Optional[python_datetime]
    last_modified_at: Optional[python_datetime]
...
def setup_logging(quiet_mode: bool):
    ...
    # Silence the chatty Smartsheet SDK logger
...
# --- Smartsheet API Interaction & Logic ---


def calculate_asset_metrics(asset_obj: Any, asset_info: AssetInfo) -> AssetMetrics:
    sname = getattr(asset_obj, 'name', asset_info.stub.name)
    sid = getattr(asset_obj, 'id', asset_info.stub.id)

    created_at_raw = getattr(asset_obj, 'createdAt', None)
    modified_at_raw = getattr(asset_obj, 'modifiedAt', None)
    ...
    rows_with_data, total_allocated_rows, cols, current_total_cells, filled_cells = 'N/A', 'N/A', 'N/A', 'N/A', 'N/A'
    row_util, col_util, shape_cell_util, filled_cell_util, ref_cnt, ref_util = 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A'
    gantt_enabled, dependencies_enabled, resource_management_enabled, from_template = 'N/A', 'N/A', 'N/A', 'N/A'
    effective_attachments = getattr(
        asset_obj, 'effectiveAttachmentCount', None)

    if asset_info.asset_type in ['Sheet', 'Report']:
        rows_with_data = len(getattr(asset_obj, 'rows', []))
        cols = len(getattr(asset_obj, 'columns', []))
        total_allocated_rows = getattr(
            asset_obj, 'totalRowCount', rows_with_data)
        current_total_cells = rows_with_data * cols
        filled_cells = sum(1 for r in getattr(asset_obj, 'rows', []) for c in getattr(
            r, 'cells', []) if getattr(c, 'value', None) not in (None, "", []))

        row_util = (total_allocated_rows / SMARTSHEET_MAX_ROWS) if isinstance(
            total_allocated_rows, (int, float)) and SMARTSHEET_MAX_ROWS > 0 else 0.0
        col_util = (cols / SMARTSHEET_MAX_COLS) if isinstance(cols,
                                                              (int, float)) and SMARTSHEET_MAX_COLS > 0 else 0.0
        ...
        shape_cell_util = (
            current_total_cells / max_cells_for_shape) if max_cells_for_shape > 0 else 0.0
        filled_cell_util = (
            filled_cells / current_total_cells) if current_total_cells > 0 else 0.0
        ref_cnt = len(getattr(asset_obj, 'crossSheetReferences', []))
        ref_util = (
            ref_cnt / SMARTSHEET_MAX_INBOUND_REFS_PER_SHEET) if SMARTSHEET_MAX_INBOUND_REFS_PER_SHEET > 0 else 0.0

        gantt_enabled = getattr(asset_obj, 'ganttEnabled', None)
        dependencies_enabled = getattr(asset_obj, 'dependenciesEnabled', None)
        resource_management_enabled = getattr(
            asset_obj, 'resourceManagementEnabled', None)
        from_template = (getattr(getattr(asset_obj, 'source', None), 'type', None)
                         == 'template') if hasattr(asset_obj, 'source') else None

    return AssetMetrics(
        asset_name=sname, asset_id=sid, asset_type=asset_info.asset_type,
        container_path=asset_info.container_path, access_level=getattr(
            asset_obj, 'accessLevel', None),
        owner=getattr(asset_obj, 'owner', None), owner_id=getattr(asset_obj, 'ownerId', None),
        created_at=created_at_dt, last_modified_at=modified_at_dt, days_since_modified=days_since_mod,
        ...
        effective_attachments=effective_attachments, gantt_enabled=gantt_enabled,
        dependencies_enabled=dependencies_enabled, resource_management_enabled=resource_management_enabled,
    ...
    try:
        if context.shutdown_event.is_set():
            return AssetMetrics(
                ...
                asset_type=asset_info.asset_type, container_path=asset_info.container_path,
                error="Analysis cancelled by user.",
                access_level=getattr(asset_info.stub, 'accessLevel', None),
                owner=None, owner_id=None,
                created_at=None, last_modified_at=None, days_since_modified=None,
                version_saves=None, permalink=getattr(asset_info.stub, 'permalink', None),
                total_allocated_rows='N/A', rows_with_data='N/A', row_util_percent='N/A',
                total_columns='N/A', col_util_percent='N/A', total_cells_current_shape='N/A',
                filled_cells='N/A', filled_cell_util_percent='N/A', shape_cell_util_percent='N/A',
                cross_sheet_refs='N/A', ref_util_percent='N/A', effective_attachments=None,
                gantt_enabled=None, dependencies_enabled=None, resource_management_enabled=None,
                from_template=None
            )

        metrics = calculate_asset_metrics(asset_info.stub, asset_info)

        if not context.quiet_mode:
            tqdm.write(
                f"📊 {metrics.asset_name[:30].ljust(30)} ({metrics.asset_type}) | "
        ...
        return AssetMetrics(
            ...
            access_level=getattr(asset_info.stub, 'accessLevel', None),
            owner=None, owner_id=None, created_at=None, last_modified_at=None, days_since_modified=None,
            version_saves=None, permalink=getattr(asset_info.stub, 'permalink', None),
            total_allocated_rows='N/A', rows_with_data='N/A', row_util_percent='N/A',
            total_columns='N/A', col_util_percent='N/A', total_cells_current_shape='N/A',
            filled_cells='N/A', filled_cell_util_percent='N/A', shape_cell_util_percent='N/A',
...
def fetch_assets_from_folder_recursive(context: ProcessingContext, folder_id: int, current_path_prefix: str, pbar_discovery: tqdm) -> List[AssetInfo]:
    ...
    try:
        folder = unwrap(context.client.Folders.get_folder(
            folder_id, include="sheets,dashboards,folders,level=2"))

        # ADDED: Check if the folder object is a valid folder and not an error
        if not hasattr(folder, 'name'):
            logging.warning(
                f"❌ Skipping inaccessible folder with ID {folder_id} within path: {current_path_prefix}")
            context.error_logs.append({
                "Item ID": folder_id,
                "Item Type": "Folder",
                "Container Path": current_path_prefix,
                "Error": "Smartsheet API returned an error object (likely due to permissions)"
            })
            return []
        ...
        if not context.quiet_mode:
            pbar_discovery.set_description(
                f" 📁 Scanning Folder: {folder.name[:25]}...")

        for asset_stub in getattr(folder, "sheets", []):
            asset_type = 'Report' if '/reports/' in getattr(
                asset_stub, 'permalink', '') else 'Sheet'
            gathered_assets.append(
                AssetInfo(stub=asset_stub, container_path=folder_path, asset_type=asset_type))

        for asset_stub in getattr(folder, "dashboards", []):
            gathered_assets.append(
                AssetInfo(stub=asset_stub, container_path=folder_path, asset_type='Dashboard'))
...
def discover_assets_in_workspaces(context: ProcessingContext, workspace_ids: List[int], discovery_limit: Optional[int]) -> List[AssetInfo]:
    ...
    logging.info("✨ Starting asset discovery...")
    pbar_kwargs = {"disable": context.quiet_mode, "total": len(
        workspace_ids), "desc": "Discovering workspaces", "unit": "item"}
    ...
    try:
        with tqdm(**pbar_kwargs) as pbar_discovery:
            for widx, wid in enumerate(workspace_ids):
                ...
                try:
                    ws = unwrap(context.client.Workspaces.get_workspace(
                        wid, include="sheets,dashboards,folders,level=2"))
                    ws_path = ws.name
                    tqdm.write(f"💼 Scanning Workspace: {ws_path} ({ws.id})")
                    pbar_discovery.update(1)

                    for asset_stub in getattr(ws, "sheets", []):
                        asset_type = 'Report' if '/reports/' in getattr(
                            asset_stub, 'permalink', '') else 'Sheet'
                        all_discovered_assets.append(
                            AssetInfo(stub=asset_stub, container_path=ws_path, asset_type=asset_type))

                    for asset_stub in getattr(ws, "dashboards", []):
                        all_discovered_assets.append(
                            AssetInfo(stub=asset_stub, container_path=ws_path, asset_type='Dashboard'))
                    ...
                    if not context.shutdown_event.is_set():
                        tqdm.write(
                            f"⚠️ Couldn’t open workspace {wid}: {e.error_code} - {e.message}")
                        context.error_logs.append(
            ...
            tqdm.write(
                f"✅ {pbar_desc}. Found {len(all_discovered_assets)} assets to analyze.")
            if pbar_discovery.n < pbar_discovery.total:
                pbar_discovery.update(pbar_discovery.total - pbar_discovery.n)
    except KeyboardInterrupt:
        tqdm.write("🛑 Discovery phase interrupted by user.")
        context.shutdown_event.set()

    if discovery_limit is not None and len(all_discovered_assets) > discovery_limit:
        tqdm.write(f"➡️ Discovery limited to first {discovery_limit} assets.")
...
def process_assets_concurrently(context: ProcessingContext, assets_to_analyze: List[AssetInfo], max_workers: int) -> List[AssetMetrics]:
    ...
    try:
                ...
                for future in as_completed(future_to_asset_info):
                    ...
                    try:
                        ...
                        error_metrics_payload.update({
                            ...
                            "container_path": s_info_item.container_path, "access_level": s_info_item.stub.accessLevel,
                            "error": f"Future execution failed: {exc}",
                            "total_allocated_rows": "N/A", "rows_with_data": "N/A", "row_util_percent": "N/A",
                            "total_columns": "N/A", "col_util_percent": "N/A", "total_cells_current_shape": "N/A",
                            "filled_cells": "N/A", "filled_cell_util_percent": "N/A", "shape_cell_util_percent": "N/A",
                            "cross_sheet_refs": "N/A", "ref_util_percent": "N/A"
                        })
        ...
        tqdm.write("🛑 Analysis interrupted by user. Shutting down workers...")
        context.shutdown_event.set()
...
def save_report_to_excel(context: ProcessingContext, output_filename: str, metrics_results: List[AssetMetrics]):
    timestamp = python_datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    tqdm.write(f"💾 Saving report to {final_outfile}...")
    ...
    try:
        with pd.ExcelWriter(final_outfile, engine='xlsxwriter') as writer:
            ...
            if not df_errors.empty:
                ...
                tqdm.write(
                    f"⚠️ {len(df_errors)} errors/issues logged. Check 'Errors and Issues' tab.")
            else:
...
def print_summary_report(metrics_results: List[AssetMetrics], context: ProcessingContext):
    ...
    if num_assets_analyzed > 0:
        summary.extend([
            ...
            f"  Average Row Utilization: {valid_metrics_df['row_util_percent'].mean():.2%}" if 'row_util_percent' in valid_metrics_df.columns and valid_metrics_df[
                'row_util_percent'].dtype != object else "N/A",
            f"  Average Column Utilization: {valid_metrics_df['col_util_percent'].mean():.2%}" if 'col_util_percent' in valid_metrics_df.columns and valid_metrics_df[
                'col_util_percent'].dtype != object else "N/A",
            f"  Average Shape Cell Utilization: {valid_metrics_df['shape_cell_util_percent'].mean():.2%}" if 'shape_cell_util_percent' in valid_metrics_df.columns and valid_metrics_df[
                'shape_cell_util_percent'].dtype != object else "N/A",
            f"  Average Filled Cell Utilization: {valid_metrics_df['filled_cell_util_percent'].mean():.2%}" if 'filled_cell_util_percent' in valid_metrics_df.columns and valid_metrics_df[
                'filled_cell_util_percent'].dtype != object else "N/A",
            f"  Average Cross-Sheet Ref Utilization: {valid_metrics_df['ref_util_percent'].mean():.2%}" if 'ref_util_percent' in valid_metrics_df.columns and valid_metrics_df[
                'ref_util_percent'].dtype != object else "N/A"
        ])

        if 'row_util_percent' in valid_metrics_df.columns and valid_metrics_df['row_util_percent'].dtype != object:
...
def main():
    ...
    try:
        ...
        if processing_ctx.shutdown_event.is_set() and not assets_to_analyze:
            tqdm.write("ℹ️ No assets discovered before interruption. Exiting.")
            if processing_ctx.error_logs:
                save_report_to_excel(processing_ctx, cli_args.output, [])
            return
        elif not assets_to_analyze:
            tqdm.write(
                "ℹ️ No assets found to analyze based on criteria. Exiting.")
            if processing_ctx.error_logs:
                save_report_to_excel(processing_ctx, cli_args.output, [])
            return

        tqdm.write(f"✅ {len(assets_to_analyze)} assets queued for analysis.")
        ...
        if analyzed_asset_metrics or processing_ctx.error_logs:
            ...
            if processing_ctx.shutdown_event.is_set():
                tqdm.write("ℹ️ Process interrupted, no data collected.")
            else:
                tqdm.write("ℹ️ No data collected, no errors. Nothing to save.")
        ...
        tqdm.write(
            f"\n⏱️ Total script execution time: {total_script_duration:.2f} seconds.")
